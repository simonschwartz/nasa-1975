...
import { NASA_DARK, NASA_LIGHT, NASA_RED, NASA_DEFAULT_LOGO_COLOR } from '../../tokens';
...
const logoStyle = size => {
  const { logoWidth } = LOGO_WIDTHS[size] || LOGO_WIDTHS["lg"];
...
const fontStyle = size => {
  const { fontSize } = FONT_SIZES[size] || FONT_SIZES["lg"];
...
const StemLogo = ({
  ...
  const logoColor = theme => {
    ...
    return NASA_DEFAULT_LOGO_COLOR;
  }
  return (
    <section
    css={wrapperStyle}
